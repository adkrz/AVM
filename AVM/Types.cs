namespace AVM
{
    ...
    /// 
    /// Conventions:
    /// read: [PARAMS] pop: [ARGUMENTS]  push: PUSHED_VALUE
    /// PARAMS are parameters read directly from program code (aka hardcoded values)
    /// ARGUMENTS are popped from stack (aka user input)
    /// PUSHED_VALUE - if instruction produces something, will be pushed to the stack
    /// 
    /// In two argument operations, like SUB or LESS, top of stack is left hand side operand, next value is rhs. 
    /// This means, e.g. result of LESS is POP() < POP().
    /// 
    /// Calling convention in CALL/RET:
    /// - push arguments to the stack, if any
    /// - push place for return value, if any. Return value can be at any place in argument list
    /// - CALL pushes stack and frame pointers
    /// - use LOAD_ARG / STORE_ARG to access arguments and return value
    /// - use LOAD_LOCAL / STORE_LOCAL to access local variables relatve to frame pointer, regardless where SP is currently
    /// - RET rewinds stack back to FP so discards all locals
    /// - caller is responsible for stack cleanup
    /// - use LOAD_LOCAL in caller to read return values, POP to discard temporary values after call etc
    /// 
    /// </summary>
    public enum I
    {
    ...
    enum Stdlib
    {
        ...
        ShowConsoleCursor,
        /// <summary>
        /// Sets background and foreground colors of the console. <see cref="Colors"/>
        /// pop 2x val8bit
        /// </summary>
        SetConsoleColors,
        /// <summary>
